-- Pin the breakdown to ONE paper exam.
--
-- `unique (edition_year, stage, title)` lets two exams share an edition and a
-- stage, so `p.exam_id in (select ... where edition_year = v_edition and stage
-- = p_stage)` could match a rep twice and list them twice — and the "scored
-- between them" total would silently double-count. The subject lookup already
-- picked a single exam; the rep join now uses that same one. Idempotent.
